...
// ── Hand geometry (reference design at scale = 1) ─────────────────────────────
// Lengths in px; multiplied by layout.scale when padding shrinks the face.
...
const HAND_REF = {
  ...
  hourWidth:   16,
  minuteTail:  32,
  minuteWidth: 13,   // slightly narrower than hourWidth
  minuteTipGap: 6,  // minute tip sits this far inside the outer minute ticks
  hubOuter:    13,
  hubInner:     5,
  faceRimInset: 5,
  tickInset:    6,
  hourTick:     { len: 28, w: 14 },
  cardinalTick: { len: 40, w: 14 },  // 12 / 3 / 6 / 9 — longer than other hour ticks
  minTick:      { len: 11, w:  5 },
};

/** Minute-hand tip distance from centre (overlaps hour ticks, near minute ticks). */
function minuteHandTip(R, scale) {
  return R - (HAND_REF.tickInset + HAND_REF.minuteTipGap) * scale;
}
...
module.exports = {
  ...
  SLOT_NUMBERS,
  SLOT_TO_CORNER,
  C_CANVAS, C_RIM, C_FACE, C_TICK, C_HAND, C_COMPLICATION_LABEL,
};
